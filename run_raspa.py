...
class RASPA_calculation(object):
    def __init__(self):
        self.IniCycles= 10000
        self.TotCycles= 100000
    ...
    def setup(self,names,p2c,calcname):
        self.cifnames = names
        self.path2cifs = p2c
        ...
        for pid,pressure in enumerate(self.ExternalPressures):
            for tid,temperature in enumerate(self.ExternalTemperatures):
                for nid,name in enumerate(self.cifnames):
                    self.write_sim_input(nid,pid,tid)
...
if __name__ == "__main__":
    ...
    # example: ../run_raspa.py filenames cifs/ test
    ...
    sim.setup (cifnames,sys.argv[2] , sys.argv[3] )
    # sim.gascomposition = {"N2":0.85,"CO2":0.15}
    sim.gascomposition = {"N2":0.793,"CO2":0.15,"water":0.057}
    # sim.ExternalTemperatures=[298]
    sim.ExternalPressures=[1e5]
    # sim.ExternalPressures=[1e3,2.5e3,5e3,7.5e3,1e4,2.5e4,5e4]
    #sim.gascomposition = {"N2":1}
    sim.ExternalTemperatures=[313]
